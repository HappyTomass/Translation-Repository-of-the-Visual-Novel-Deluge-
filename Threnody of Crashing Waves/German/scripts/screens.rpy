# TODO: Translation updated at 2025-09-29 19:17

translate german strings:

    # game/screens.rpy:304
    old "Start"
    new "Start"

    # game/screens.rpy:308
    old "History"
    new "Verlauf"

    # game/screens.rpy:310
    old "Save"
    new "Speichern"

    # game/screens.rpy:312
    old "Load"
    new "Laden"

    # game/screens.rpy:314
    old "Preferences"
    new "Einstellungen"

    # game/screens.rpy:318
    old "End Replay"
    new "Wiederholung beenden"

    # game/screens.rpy:322
    old "Main Menu"
    new "Hauptmenü"

    # game/screens.rpy:324
    old "About"
    new "Über"

    # game/screens.rpy:328
    old "DLC Chapter"
    new "DLC-Kapitel"

    # game/screens.rpy:334
    old "Help"
    new "Hilfe"

    # game/screens.rpy:340
    old "Quit"
    new "Beenden"

    # game/screens.rpy:481
    old "Return"
    new "zurück"

    # game/screens.rpy:565
    old "Version [config.version!t]\n"
    new "Version [config.version!t]\n"

    # game/screens.rpy:732
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "Erstellt mit {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"

    # game/screens.rpy:768
    old "Page {}"
    new "Seite {}"

    # game/screens.rpy:768
    old "Automatic saves"
    new "automatische Sicherungen"

    # game/screens.rpy:768
    old "Quick saves"
    new "schnelle Sicherungen"

    # game/screens.rpy:810
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%A, der %d. %B %Y,\n%H:%M Uhr"

    # game/screens.rpy:810
    old "empty slot"
    new "leerer Speicherplatz"

    # game/screens.rpy:827
    old "<"
    new "<"

    # game/screens.rpy:830
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:833
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:839
    old ">"
    new ">"

    # game/screens.rpy:894
    old "Language"
    new "Sprache"

    # game/screens.rpy:906
    old "Display"
    new "Anzeige"

    # game/screens.rpy:907
    old "Window"
    new "Fenster"

    # game/screens.rpy:908
    old "Fullscreen"
    new "Vollbild"

    # game/screens.rpy:912
    old "Rollback Side"
    new "Rollback-Seite"

    # game/screens.rpy:913
    old "Disable"
    new "deaktivieren"

    # game/screens.rpy:914
    old "Left"
    new "links"

    # game/screens.rpy:915
    old "Right"
    new "rechts"

    # game/screens.rpy:919
    old "Skip"
    new "spulen"

    # game/screens.rpy:920
    old "Unseen Text"
    new "ungelesener Text"

    # game/screens.rpy:921
    old "After Choices"
    new "nach Entscheidungen"

    # game/screens.rpy:922
    old "Transitions"
    new "Übergänge"

    # game/screens.rpy:926
    old "Auto-Forward"
    new "automatisches Textumblättern"

    # game/screens.rpy:927
    old "Auto"
    new "auto"

    # game/screens.rpy:940
    old "Text Speed"
    new "Textgeschwindigkeit"

    # game/screens.rpy:944
    old "Auto-Forward Time"
    new "Zeit des automatischen Textumblätterns"

    # game/screens.rpy:951
    old "Music Volume"
    new "Musiklautstärke"

    # game/screens.rpy:958
    old "Sound Volume"
    new "Soundlautstärke"

    # game/screens.rpy:964
    old "Test"
    new "Test"

    # game/screens.rpy:968
    old "Voice Volume"
    new "Stimmenlautstärke"

    # game/screens.rpy:979
    old "Mute All"
    new "alles stummschalten"

    # game/screens.rpy:1098
    old "The dialogue history is empty."
    new "Der Dialogverlauf ist leer."

    # game/screens.rpy:1168
    old "Keyboard"
    new "Tastatur"

    # game/screens.rpy:1169
    old "Mouse"
    new "Maus"

    # game/screens.rpy:1172
    old "Gamepad"
    new "Gamepad"

    # game/screens.rpy:1185
    old "Enter"
    new "Enter"

    # game/screens.rpy:1186
    old "Advances dialogue and activates the interface."
    new "Fördert den Dialog und aktiviert die Schnittstelle."

    # game/screens.rpy:1189
    old "Space"
    new "Leertaste"

    # game/screens.rpy:1190
    old "Advances dialogue without selecting choices."
    new "Fördert den Dialog, ohne Entscheidungen zu treffen."

    # game/screens.rpy:1193
    old "Arrow Keys"
    new "Pfeiltasten"

    # game/screens.rpy:1194
    old "Navigate the interface."
    new "Bedient die Benutzeroberfläche."

    # game/screens.rpy:1197
    old "Escape"
    new "ESC"

    # game/screens.rpy:1198
    old "Accesses the game menu."
    new "Öffnet das Spielmenü."

    # game/screens.rpy:1201
    old "Ctrl"
    new "Strg"

    # game/screens.rpy:1202
    old "Skips dialogue while held down."
    new "Spult den Dialog, während die Taste gedrückt wird."

    # game/screens.rpy:1205
    old "Tab"
    new "Tab"

    # game/screens.rpy:1206
    old "Toggles dialogue skipping."
    new "(De-)Aktiviert das Dialogspulen."

    # game/screens.rpy:1209
    old "Page Up"
    new "Bild hoch"

    # game/screens.rpy:1210
    old "Rolls back to earlier dialogue."
    new "Blättert eine Seite zurück."

    # game/screens.rpy:1213
    old "Page Down"
    new "Bild runter"

    # game/screens.rpy:1214
    old "Rolls forward to later dialogue."
    new "Blättert eine Seite vor."

    # game/screens.rpy:1218
    old "Hides the user interface."
    new "Blendet die Benutzeroberfläche aus."

    # game/screens.rpy:1222
    old "Takes a screenshot."
    new "Nimmt ein Bildschirmfoto auf."

    # game/screens.rpy:1226
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "(De-)Aktiviert das unterstützende {a=https://www.renpy.org/l/voicing}Vorlesen{/a}."

    # game/screens.rpy:1232
    old "Left Click"
    new "Linksklick"

    # game/screens.rpy:1236
    old "Middle Click"
    new "Mittelklick"

    # game/screens.rpy:1240
    old "Right Click"
    new "Rechtsklick"

    # game/screens.rpy:1244
    old "Mouse Wheel Up\nClick Rollback Side"
    new "hochscrollen\nauf die Rollback Seite klicken"

    # game/screens.rpy:1248
    old "Mouse Wheel Down"
    new "runterscrollen"

    # game/screens.rpy:1255
    old "Right Trigger\nA/Bottom Button"
    new "rechter Trigger\nA/unterer Knopf"

    # game/screens.rpy:1259
    old "Left Trigger\nLeft Shoulder"
    new "linker Trigger\nlinke Schultertaste"

    # game/screens.rpy:1263
    old "Right Shoulder"
    new "rechte Schultertaste"

    # game/screens.rpy:1268
    old "D-Pad, Sticks"
    new "Steuerkreuz, Sticks"

    # game/screens.rpy:1272
    old "Start, Guide"
    new "Start, Guide"

    # game/screens.rpy:1276
    old "Y/Top Button"
    new "Y/obere Taste"

    # game/screens.rpy:1279
    old "Calibrate"
    new "kalibrieren"

    # game/screens.rpy:1344
    old "Yes"
    new "Ja"

    # game/screens.rpy:1345
    old "No"
    new "Nein"

    # game/screens.rpy:1391
    old "Skipping"
    new "spulen"

    # game/screens.rpy:1611
    old "Back"
    new "zurück"

    # game/screens.rpy:1614
    old "Menu"
    new "Menü"
